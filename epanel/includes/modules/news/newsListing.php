...
$query = "SELECT * FROM plug_newsslider";
...
<div class="content_in">
	<div>
		<h3 class="sub-head">
			<img src="<?php echo baseUrl; ?>images/news-icon.jpg" alt=""
				width="79" height="60" align="absmiddle" style="margin-right: 10px;">News
		</h3>
	</div>
	...
	<table width="950" border="0" cellspacing="0" cellpadding="0"
		...
		<?php while ($row = mysqli_fetch_array($res)) {?>
			<tr id="option_id_<?php echo $row['id']; ?>" class="<?php if($i%2 == 0){ echo 'even'; } else { echo 'odd'; }?>">
				<td>
					<a href="#" onClick="deleteContent('news',<?php echo $row['id']; ?>)" >
						<img src="<?php echo baseUrl; ?>images/delet-icon.png" alt="" width="24" height="24">
					</a>	
				</td>
				<td>
					<input type="checkbox" name="active" id="active" onClick="updateActiveContent('news',<?php echo $row['id']; ?>,this)"
					<?php if(1==$row['active']) { echo 'checked'; } ?> value="<?php if(1==$row['active']) { echo '1'; } else { echo '0'; } ?>" />
				</td>
				<td>
					<a href="#" onClick="editContent('news','news',<?php echo $row['id']; ?>)">
